    ...
    ReverseClaimer
{
    ...
    bytes32 private ETH_NODE;

    bytes32 private ETH_LABELHASH;
    ...
    constructor(
        ...
        ETH_NODE = nameHash(_name);
        ETH_LABELHASH = labelHash(_name);
        bytes memory nameLength = abi.encodePacked(bytes1(uint8(bytes(_name).length)));
        ...
        names[ROOT_NODE] = "\x00";
        names[ETH_NODE] = abi.encodePacked(nameLength, _name, "\x00");
    }                   

    function labelHash(string memory _label) internal pure returns (bytes32) {
        return keccak256(
            abi.encodePacked(_label)
        );
    }
    ...
    function nameHash(string memory _name) internal pure returns (bytes32) {
        if (bytes(_name).length == 0) {
            ...
            (label, remainder) = splitLabel(_name);
            return keccak256(abi.encodePacked(nameHash(string(remainder)), keccak256(label)));
        }
    }

    function splitLabel(string memory _name) internal pure returns (bytes memory, bytes memory) {
        bytes memory label;
        bytes memory remainder;
        bytes memory nameBytes = bytes(_name);
        uint256 dotIndex = findDotIndex(nameBytes);
        
        if (dotIndex == nameBytes.length) {
            label = nameBytes;
        } else {
            label = new bytes(dotIndex);
            remainder = new bytes(nameBytes.length - dotIndex - 1);
            ...
            for (uint256 i = dotIndex + 1; i < nameBytes.length; i++) {
                remainder[i - dotIndex - 1] = nameBytes[i];
            }
        }
    ...
    function findDotIndex(bytes memory _nameBytes) internal pure returns (uint256) {
        for (uint256 i = 0; i < _nameBytes.length; i++) {
            if (_nameBytes[i] == bytes1('.')) {
                return i;
            }
        }
        return _nameBytes.length;
    }
