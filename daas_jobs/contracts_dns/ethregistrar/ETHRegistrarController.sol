    ...
    ReverseClaimer
{
    ...
    function setPrice(IPriceOracle _prices) external onlyOwner {
        ...
        require(address(_prices) != address(0x0), "ERROR: Prices is zero address");
    ...
    function rentPrice(
        ...
        price = prices.price(name, base.nameExpires(uint256(label)), duration, user);
    }
    ...
    function commit(bytes32 commitment) public override {
        require(commitments[commitment] + maxCommitmentAge < block.timestamp, "ERROR: UnexpiredCommitmentExists");
    ...
    function _consumeCommitment(
        ...
        require(commitments[commitment] + minCommitmentAge < block.timestamp, "ERROR: CommitmentTooNew");

        // If the commitment is too old, or the name is registered, stop
         require(commitments[commitment] + maxCommitmentAge > block.timestamp, "ERROR: Commit to old");
         require(available(name), "ERROR: NameNotAvailable");

        delete (commitments[commitment]);

        require(duration > MIN_REGISTRATION_DURATION,"ERROR: DurationTooShort");
        // }
